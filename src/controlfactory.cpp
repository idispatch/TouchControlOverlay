...
bool getButtonProperty(xmlAttr *properties, const char *propertyName, int& value) {
    if (!xmlStrncasecmp(properties->name, BAD_CAST propertyName, strlen(propertyName))) {
        if (properties->children && properties->children->content) {
            ...
            } else if (!xmlStrncasecmp(properties->children->content, BAD_CAST "right",
                    strlen("right"))) {
                value = TCO_MOUSE_RIGHT_BUTTON;
            } else if (!xmlStrncasecmp(properties->children->content, BAD_CAST "middle",
                    strlen("middle"))) {
    ...
    while (child) {
        if (!xmlStrncasecmp(child->name, BAD_CAST "label", strlen("label"))) {
            ...
            while (properties) {
                ...
                getProperty(properties, "width", w);
                getProperty(properties, "height", h);
                getProperty(properties, "alpha", a);
                if (!xmlStrncasecmp(properties->name, BAD_CAST "image", strlen("image"))) {
                    if (properties->children && properties->children->content)
            ...
            label = new Label(context->screenContext(), x, y, w, h, a, (char*) childImage);
...
Control *ControlFactory::createControl(TCOContext *context, int controlType, int x, int y, int w,
        int h, std::stringstream &ss) {
    int extra[3];
    uint16_t unicode;
    Control *control = 0;
    ...
    case Control::KEY:
        ...
        control = new Control(context->screenContext(), type, x, y, w, h,
                new KeyEventDispatcher(context->handleKeyFunc(), extra[0], extra[1], extra[2],
                        unicode));
        break;
    case Control::DPAD:
        control = new Control(context->screenContext(), type, x, y, w, h,
    ...
    case Control::MOUSEBUTTON:
        ss >> extra[0] >> extra[1];
        control = new Control(context->screenContext(), type, x, y, w, h,
                new MouseButtonEventDispatcher(context->handleMouseButtonFunc(), extra[0],
                        extra[1]));
        break;
    default:
        return 0;
